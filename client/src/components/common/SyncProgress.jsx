...
export default function SyncProgress({ active, result, progressCompany, livePhase }) {
  ...
  const statusFor = (step) => {
    if (result && !active) {
      ...
      // Sales / Purchase Register may have been fanned into per-quarter
      // shards (and per-FY suffixes for multi-FY tenants). Treat the
      // row as errored if any shard errored, done if any shard fetched
      // cleanly. Same rollup pattern works for any register key whose
      // shards share the prefix.
      if (step.key === 'salesRegister' || step.key === 'purchaseRegister') {
        const errored = [...errKeys].filter((k) => k === step.key || k.startsWith(`${step.key}_`));
        const fetched = [...fetchedKeys].filter((k) => k === step.key || k.startsWith(`${step.key}_`));
        if (errored.length && !fetched.length) return 'error';
        if (errored.length) return 'error';
        if (fetched.length) return 'done';
        return totalFailure ? 'error' : 'pending';
      }
    ...
    if (hasLiveDriver) {
      ...
      // Sales / Purchase Register collapse every per-quarter / per-FY
      // shard (salesRegister, salesRegister_FY26-27_Q1, ...) into one
      // row. Running if any shard is running, error if any shard
      // errored, done if every shard finished cleanly.
      if (step.key === 'salesRegister' || step.key === 'purchaseRegister') {
        const matches = (k) => k === step.key || k.startsWith(`${step.key}_`);
        const shardKeys = Object.keys(livePhase.keyStatus).filter(matches);
        if (!shardKeys.length) {
          if (livePhase.currentKey && matches(livePhase.currentKey)) return 'running';
          return 'pending';
        }
        if (shardKeys.some((k) => livePhase.keyStatus[k] === 'running')) return 'running';
        if (shardKeys.some((k) => livePhase.keyStatus[k] === 'error')) {
          const allFinished = shardKeys.every((k) => livePhase.keyStatus[k] !== 'running');
          return allFinished ? 'error' : 'running';
        }
        if (shardKeys.every((k) => livePhase.keyStatus[k] === 'done')) return 'done';
        return 'pending';
      }
  ...
  const detailFor = (step) => {
    ...
    // Sales / Purchase Register may have been fanned out into
    // per-quarter / per-FY shards — sum every shard so the single
    // register row shows the combined total.
    const sumShards = (baseKey) => Object.entries(result.counts || {})
      .filter(([k]) => k === baseKey || k.startsWith(`${baseKey}_`))
      .reduce((s, [, v]) => s + (Number(v) || 0), 0);
    const salesTotal = sumShards('salesRegister');
    const purchaseTotal = sumShards('purchaseRegister');
    const countMap = {
      ledgers: result.ledgers,
      accountingGroups: result.counts?.accountingGroups,
      salesRegister: salesTotal || result.counts?.salesRegister,
      purchaseRegister: purchaseTotal || result.counts?.purchaseRegister,
